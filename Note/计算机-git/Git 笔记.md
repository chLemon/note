...
[alias]
        ...
        # log 相关
        lg   = log --color --graph --pretty=format:'%Cred%h%Creset-%C(yellow)%d%Creset %s %Cgreen(%cd)%Creset %C(bold blue)<%an>%Creset' --abbrev-commit --date=format:'%Y-%m-%d %H:%M:%S'
        lgme = lg --author=chLemon
...
[github 中文官方文档](https://docs.github.com/cn/github)
...
只需要将 `www.github.com/` 后面为代码库 改为 `www.github.com.cnpmjs.org/后面为代码库地址`

#### 5.7.2. 通过 Gitee 中转 fork 仓库加速下载
...
```shell
git log --since="2022-10-01" --before="2023-01-01" --author="chLemon" --pretty=tformat: --numstat | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "added lines: %s, removed lines: %s, total lines: %s", add, subs, loc }'
```
...
### 9.1. RPC failed; curl 56 OpenSSL SSL_read: SSL_ERROR_SYSCALL, errno 10054
...
### 9.2. warning: LF will be replaced by CRLF in ball_pool/assets/Main.js.
...
### 9.4. ssh: connect to host github.com port 22: Connection refused
